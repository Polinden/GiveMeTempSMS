...
#define WAITNET 20
#define INITS {"AT+CMGF=1", "AT+CNMI=1,2,0,0,1", "AT+CMGDA=\"DEL ALL\"", "AT+GSMBUSY=1"}
...
void setup() {
	...
	for (auto i=0; i<WAITNET; i++){
		Serial.print('.');   
		delay(1000);     
	};
	Serial.println('.'); 
	const char * initString [] =INITS;	
	smska = new MySMS(&altSerial, &Serial, initString, TIMEOUT);
